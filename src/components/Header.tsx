    ...
    <header className="fixed w-full bg-white/95 backdrop-blur-sm z-50 border-b border-stone-200">
      <div className="max-w-6xl mx-auto px-6 py-2 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <a href="/availability" className="text-stone-600 hover:text-stone-900 transition-colors text-sm">Available Units</a>
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-stone-200 px-6 py-4 space-y-4">
          ...
          <a href="/availability" className="block text-stone-600 hover:text-stone-900 text-sm" onClick={() => setMenuOpen(false)}>Available Units</a>
          <button
            onClick={() => {
              document.getElementById('location-section')?.scrollIntoView({ behavior: 'smooth' });
              onLocationClick?.();
